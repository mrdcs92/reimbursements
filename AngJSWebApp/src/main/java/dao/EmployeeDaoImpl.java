...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	public Employee employeeLogin(String email, String password) {
		...
		try {
			...
			while (rs.next()) {
				employee = new Employee(rs.getInt("user_id"), rs.getString("username"), rs.getString("password"),
						rs.getString("email"), reimbursements);
			}
			ps.close();
	...
	public List<Reimbursement> resolvedReimbursements(int employeeId) {
		...
		try {
			...
			String sql = "select "
					...
					+ "join managers m on m.user_id = rem.man_id " + "where rem.emp_id = ?";
	...
	public Employee getCredentials(int employeeId) {
		...
		try {
			...
			String sql = "select username, password, email " + "from " + "employees " + "where user_id = ?";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, employeeId);
			ResultSet rs = ps.executeQuery();

			while (rs.next()) {
				employee = new Employee(employeeId, rs.getString("username"), rs.getString("password"),
						rs.getString("email"), reimbursements);
			}
			ps.close();
			return employee;
